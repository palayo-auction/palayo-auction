...
public class DepositHistoryService {
	...
	// 다건 조회 (페이징 처리 포함)
	@Transactional(readOnly = true)
	public Page<DepositHistoryResponse> getDepositHistoryList(Long auctionId, Long userId, String status,
		Pageable pageable) {
	...
	public DepositHistoryResponse createDepositHistory(DepositHistoryRequest depositHistoryRequestDTO) {
		...
		// DepositHistory 생성
		DepositHistory depositHistory = new DepositHistory(auction, user, depositHistoryRequestDTO.getDeposit(),
			depositHistoryRequestDTO.getStatus());
	...
	// 사용자가 해당 경매에 보증금을 이미 납부했는지 확인 -> 추가 요청
	@Transactional(readOnly = true)
	...
	// 유저 ID, 경매 ID, 보증금 금액을 받아 보증금 이력을 생성 -> 추가 요청
	@Transactional
